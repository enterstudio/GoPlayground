...
type session struct {
	...
	StartTime int64  `json:"startUnixTime"`
	ValidTime int    `json:"validityDuration"`
}

var tpl *template.Template

var dbUsers = map[string]user{}      // User ID, user
var dbSession = map[string]session{} // Session ID, Session Object {User ID (email)}
var mutSession sync.Mutex            // Mutex to synchronise the sessions Read write
...
func signup(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		mutSession.Lock()
		dbSession[c.Value] = NewSession(e, 20)
		mutSession.Unlock()
...
func logout(w http.ResponseWriter, r *http.Request) {
	...
	mutSession.Lock()
	e := dbSession[c.Value]
	fmt.Println("User Logged Out(Logout): ", e.UserId)
...
func NewSession(UserId string, dur int) session {
	var s session
	s.UserId = UserId
